...
Персональная landing page-презентация frontend/full-stack разработчика Arthur Dadalian. Проект подготовлен как тестовое задание: есть адаптивный React-интерфейс, Express API, рабочий feedback flow, SMTP-отправка писем, AI helper и конфигурация для деплоя на Vercel.
...
4. Nodemailer отправляет письмо владельцу сайта на `OWNER_EMAIL`.
5. Nodemailer отправляет copy email пользователю.
6. API возвращает понятный JSON.
7. Frontend показывает success или user-friendly error и очищает форму после успеха.

Если SMTP не настроен, API возвращает `503` с безопасным сообщением и не раскрывает секреты.
...
`OPENAI_API_KEY` необязателен. SMTP-переменные нужны для реальной отправки feedback emails.
